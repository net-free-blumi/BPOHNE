/**
 * Cloudflare Pages Function – OG tags למוצרים
 * כשבוט (וואטסאפ, פייסבוק וכו') מבקש /product/[id], מחזירים HTML עם meta tags מותאמים.
...
export const onRequestGet = async (context: PagesFunctionContext) => {
  ...
  // בוט – צריך OG tags. צריך Firebase Service Account
  const saJson = env.FIREBASE_SERVICE_ACCOUNT;
  if (!saJson || !id) {
    // fallback: HTML בסיסי בלי פרטי מוצר
    return new Response(
  ...
  try {
    ...
    // קריאה ל־Firestore
    const firestoreUrl = `https://firestore.googleapis.com/v1/projects/${project_id}/databases/(default)/documents/products/${id}`;
    ...
    if (!fsRes.ok) {
      ...
      throw new Error(`Firestore error: ${fsRes.status}`);
    }

    const doc = (await fsRes.json()) as { fields?: Record<string, { stringValue?: string; integerValue?: string; doubleValue?: string }> };
    ...
    const getNum = (k: string) => {
      ...
      if (f.doubleValue != null) return parseFloat(f.doubleValue);
      return null;
    };
    ...
    const imagesRaw = (doc as { fields?: { images?: { arrayValue?: { values?: { stringValue?: string }[] } } } }).fields?.images?.arrayValue?.values;
    ...
    const descOg = description ? escapeHtml(description.slice(0, 200)) : (priceStr ? escapeHtml(priceStr) : "B-Phone ביפון – תקשורת סלולרית");
